...
public class TextProcessorIntegrationTest extends StanfordNLPIntegrationTest {
    ...
    public void setUp() throws Exception {
        super.setUp();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:'tokenizer',textProcessor:{processor}, processingSteps:{tokenize:true}})", Collections.singletonMap("processor", StanfordTextProcessor.class.getName()), emptyConsumer());
    }
    ...
    public void testCustomPipelineWithoutPhraseStepShouldNotExtractPhrases() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:\"customie\", stopWords:\"hello,build\", textProcessor:'"+ StanfordTextProcessor.class.getName() +"', processingSteps:{tokenize:true, dependency:true, coref: true}})", (result -> {
            assertTrue(result.hasNext());
        }));
    ...
    public void testAnnotationWithPipelineFromUserConfig() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:\"customie\", stopWords:\"hello,build\", textProcessor:'"+ StanfordTextProcessor.class.getName() +"', processingSteps:{tokenize:true, dependency:true, coref: false}})", (result -> {
            assertTrue(result.hasNext());
        }));
        String text = "Neo4j is built from the ground up to be a graph database.";
        try (Transaction tx = getDatabase().beginTx()) {
    ...
    public void testAnnotationWithPipelineWithoutNERProcessingStep() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:\"customie\", stopWords:\"hello,build\", textProcessor:'"+ StanfordTextProcessor.class.getName() +"', processingSteps:{tokenize:true, ner: false}})", (result -> {
            assertTrue(result.hasNext());
        }));
    ...
    public void testAnnotationWithPipelineWithNERProcessingStep() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:\"customie\", stopWords:\"hello,build\", textProcessor:'"+ StanfordTextProcessor.class.getName() +"', processingSteps:{tokenize:true, ner: true}})", (result -> {
            assertTrue(result.hasNext());
        }));
        String text = "My name is John Doe and I work in Switzerland";
        executeInTransaction("CALL ga.nlp.annotate({text:'"+text+"', pipeline:'customie', id:'test', checkLanguage:false})", (result -> {
    ...
    public void testAnnotationWithPipelineWithExcludedNER() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name:\"customie\", stopWords:\"hello,build\", textProcessor:'"+ StanfordTextProcessor.class.getName() +"', processingSteps:{tokenize:true, ner: true}, excludedNER:['LOCATION']})", (result -> {
            assertTrue(result.hasNext());
        }));
        String text = "My name is John Doe and I work in Switzerland";
        executeInTransaction("CALL ga.nlp.annotate({text:'"+text+"', pipeline:'customie', id:'test', checkLanguage:false})", (result -> {
    ...
    public void testAnnotationWithCustomStopwords() {
        ...
        String query = "CALL ga.nlp.processor.addPipeline({language:'en', textProcessor: 'com.graphaware.nlp.processor.stanford.StanfordTextProcessor', name: 'customStopwords', stopWords: {stopwords} })";
    ...
    public void testAnnotationWithRelationStep() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name: 'relationsXYZ', textProcessor: {p0}, processingSteps:{tokenize:true, ner:true, dependency: true, coref:true, phrase: true, relations:true}})", buildSeqParameters(StanfordTextProcessor.class.getName()), emptyConsumer());
    ...
    public void testAnnotationWithWhitelist() {
        clearDb();
        executeInTransaction("CALL ga.nlp.processor.addPipeline({language:'en', name: 'whitelist', whitelist:'i,be,john,hello,ibm', textProcessor: {p0}, processingSteps:{tokenize:true, ner:true}})", buildSeqParameters(StanfordTextProcessor.class.getName()), emptyConsumer());
