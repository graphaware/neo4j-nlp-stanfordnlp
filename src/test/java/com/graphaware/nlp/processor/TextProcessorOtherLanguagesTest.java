...
public class TextProcessorOtherLanguagesTest {
    ...
    public static void init() {
        ...
        pipelineSpecification.setLanguage("german");
    ...
    @Test
    public void testNE() {
        String testText = "Die Posener Reden waren zwei Geheim reden, die der Reichs-führer SS Heinrich Himmler am 4. und 6.";
        AnnotatedText annotatedText = textProcessor.annotateText(testText, "de", PIPELINE_DEFAULT);
        TestAnnotatedText test = new TestAnnotatedText(annotatedText);

        test.assertSentencesCount(2);
        assertEquals("Heinrich Himmler", test.getTagAtPosition(0, 68).getLemma());
    }
...
//        specification.addProcessingStep("relations");
    ...
    public void testAnnotatedShortText() {
        AnnotatedText annotatedText = textProcessor.annotateText("Fixieren des Batch-Endpunktprotokollierungsproblems", "de", PIPELINE_DEFAULT);

        assertEquals(1, annotatedText.getSentences().size());
        TestAnnotatedText test = new TestAnnotatedText(annotatedText);
        test.assertSentencesCount(1);
        test.assertTagsCountInSentence(3, 0);
    }
