...
public abstract class StanfordNLPIntegrationTest extends NLPIntegrationTest {
    ...
    protected void createDocumentAndAnnotate(String text) {
        executeInTransaction("CREATE (n:Document) SET n.text = {p0} WITH n CALL ga.nlp.annotate({text: n.text, id: id(n), pipeline: 'default'}) YIELD result MERGE (n)-[:HAS_ANNOTATED_TEXT]->(result)", buildSeqParameters(text), emptyConsumer());
    }
